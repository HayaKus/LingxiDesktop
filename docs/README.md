...
桌面伙伴（Lingxi）是一个创新的桌面AI助手，以可爱的金毛犬形象出现在用户桌面上。它的核心创新在于**屏幕感知能力** - AI能够"看到"用户的屏幕内容,从而提供更智能、更贴合场景的辅助。
...
� **MCP协议支持** ⭐ NEW
- 集成官方MCP SDK (@modelcontextprotocol/sdk v1.25.2)
- 支持HTTP/SSE多种传输协议
- 完整的OAuth 2.1 PKCE认证流程
- 授权服务器自动发现(RFC 9728)
- 动态客户端注册(RFC 7591)
- Resource Indicators支持(RFC 8707)
- Token自动刷新和管理
- 54个AI工具可用(6个本地 + 48个MCP)
- 工具智能分类和调用
- MCP服务器配置管理界面
...
**场景**：需要调用企业内部工具和服务
- 询问："帮我查询这个实验的配置"
- AI自动调用MCP工具(如taoab实验查询)
- 返回结构化的结果数据
- 支持54个工具(包括Skyline、Taoab、Aone等)
...
#### MCP服务器管理 ⭐ NEW
- **打开MCP配置**：在设置窗口中点击"MCP服务器"标签
- **添加服务器**：点击"添加MCP服务器"按钮
- **OAuth授权**：对于需要认证的服务器，点击"授权"按钮
- **查看工具**：连接后可查看该服务器提供的工具列表
- **启用/禁用**：通过开关控制服务器连接状态
...
| MCP协议 | 不支持 | 完整支持(OAuth 2.1) |
| 工具集成 | 有限 | 54个工具(本地+MCP) |
...
MCP (Model Context Protocol) 是一个开放协议，允许AI应用连接到各种数据源和工具服务器。通过MCP，桌面伙伴可以：
- 调用企业内部服务(Skyline、Taoab、Aone等)
- 访问外部API和数据库
- 执行复杂的业务操作
- 扩展AI助手的能力边界
...
- 详细文档：docs/MCP_AUTH_FIX_SUMMARY.md

#### 3. 多服务器支持
- 同时连接多个MCP服务器
- 每个服务器独立配置
- 工具名称自动分类(mcp__server-id__tool-name)
- 智能路由工具调用

#### 4. 安全特性
- Token与服务器绑定(防止token重放)
- PKCE防止授权码拦截
- State参数防止CSRF攻击
- 本地加密存储tokens
- HTTPS强制要求
...
MCP工具自动集成到AI对话中：
```
用户：帮我查询display域下的实验列表

AI：好的，我来调用taoab查询工具...
[自动调用 mcp__formal-taoab__query_experiment]
根据查询结果，找到以下实验：...
```

### 当前集成的MCP服务器

- **formal-skyline**: 圣剑配置管理(15个工具)
- **formal-taoab**: 实验平台管理(10个工具)
- **formal-aone**: 项目管理(23个工具)
- **formal-code**: 代码管理
- **formal-ding-doc**: 文档管理
...
- ✅ 用户完全控制数据流
- ✅ BUC认证安全可靠
- ✅ MCP OAuth tokens加密存储
- ✅ Token与服务器严格绑定

### 权限说明
- **屏幕录制权限**：用于截取屏幕内容
- **粘贴板访问权限**：用于读取粘贴板截图
- **网络权限**：用于调用千问API、BUC认证和MCP服务器
