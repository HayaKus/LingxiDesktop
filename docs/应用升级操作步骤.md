# 应用升级操作步骤

## 📝 以后发布新版本

```bash
# 0. 先提交代码到内网仓库

# 1. 更新版本号
npm version 0.1.5

# 2. 构建
npm run electron:build

# 3. 更新 version.json
# 修改 version: "0.1.2"
# 修改 downloadUrl 中的版本号

# 4. 提交
git add package.json version.json
git commit -m "发布 v0.1.5"
git push github master

# 5. 在 GitHub 创建新的 Release (v0.1.2)
# 6. 上传新的安装包
```

**然后在 GitHub 发布:**

1. 访问: https://github.com/HayaKus/IamDog/releases/new

2. 填写信息:
   - **Tag version**: `v0.1.1`
   - **Release title**: `v0.1.1 - 首次发布`
   - **Description**:
     ```markdown
     ## 🎉 首次发布
     
     ### 新功能
     - ✨ 应用内检测更新功能
     - ✨ 支持版本对比和更新提示
     - ✨ 改进用户界面
     
     ### 下载
     - macOS: lingxi-0.1.1.dmg
     ```

3. **上传安装包**:
   - 点击 "Attach binaries"
   - 上传 `release/桌面伙伴-0.1.1.dmg`
   - 重命名为: `lingxi-0.1.1.dmg`

4. **点击 "Publish release"**

## ✅ 完成!

## � 更新 Homebrew Tap（可选）

如果你已经设置了 Homebrew Tap，在 GitHub 发布后需要更新：

```bash
# 方法1: 使用自动化脚本（推荐）
./scripts/calculate-sha256.sh release/桌面伙伴-0.1.5.dmg  # 计算 SHA256
./scripts/update-homebrew-tap.sh 0.1.5 <SHA256值>    # 更新 Tap

# 方法2: 手动更新
cd ~/Code/homebrew-iamdog
# 编辑 Casks/iamdog.rb，更新版本号和 SHA256
git add Casks/iamdog.rb
git commit -m "Update to version 0.1.5"
git push origin main
```

详见：[Homebrew发布指南.md](./Homebrew发布指南.md)

## �🎯 重要链接

- **你的仓库**: https://github.com/HayaKus/IamDog
- **Releases**: https://github.com/HayaKus/IamDog/releases
- **version.json**: https://raw.githubusercontent.com/HayaKus/IamDog/master/version.json
- **Homebrew Tap**: https://github.com/HayaKus/homebrew-iamdog

## 💡 小提示

### 如果推送失败

```bash
# 方法1: 使用 HTTPS (需要 GitHub Token)
git remote set-url github https://ghp_你的token@github.com/HayaKus/IamDog.git

# 方法2: 使用 SSH (推荐)
git remote set-url github git@github.com:HayaKus/IamDog.git
```

### 查看当前分支

```bash
git branch
# 如果显示 * main,就用 main
# 如果显示 * master,就用 master
```

## 🎉 全部配置完成!

现在你有:
- ✅ 代码托管在 GitHub (https://github.com/HayaKus/IamDog)
- ✅ 免费的更新服务器
- ✅ 用户无需账号即可下载更新
- ✅ 全球 CDN 加速

需要帮助随时找我! 🚀
